...
EXTERN_C  void WINAPI __w3curlparse(const char *szurl,
	...
	public:
		...
		typedef struct __HTTP_ARG {
			...
			__HTTP_ARG(const char *szn, const int v): name(szn), f(false) { char t[15]="\0"; sprintf_s(t, "%d", v); value=t; }
			__HTTP_ARG(const char *szn, const long v): name(szn), f(false) { char t[15]="\0"; sprintf_s(t, "%d", v); value=t; }
			__HTTP_ARG(const char *szn, const float v): name(szn), f(false) { char t[15]="\0"; sprintf_s(t, "%f", v); value=t; }
